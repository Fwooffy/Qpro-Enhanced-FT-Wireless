...
4. Double-click `Launch-QproWireless.cmd`. Use the Hub's normal **Prepare gaze**, capture, and tracking controls. The Hub's old **Quest USB** label means ADB is connected; it may say USB while you are wireless.
...
The modified files are `build-and-run.ps1`, `train-latest-tongue-refinement.ps1`, `train-latest-tongue-stills.ps1`, `train_tongue_model.py`, `tongue_model_preview.py`, and `native_raw_eye_probe.py`. The AMD runtime installer and wireless pair/connect/setup/launch/disable helpers are added. The executable is the original v0.1.10-poc build, so its window title still says QproFaceTracking Hub. `SHA256SUMS.txt` lists the exact contents of this AMD Wireless Edition zip.
